...
if ( 1 === $recurrence_type ) {
	$interval_length  = 15;
	$recurrence_label = __( 'day', 'buddyboss-pro' );
} else if ( 2 === $recurrence_type ) {
	$interval_length  = 12;
	$recurrence_label = __( 'week', 'buddyboss-pro' );
} else if ( 3 === $recurrence_type ) {
	$interval_length  = 3;
	$recurrence_label = __( 'month', 'buddyboss-pro' );
} else {
	$interval_length  = 0;
	$recurrence_label = __( 'day', 'buddyboss-pro' );
}
...
<div class="bp-meeting-fields-wrap">
	<div class="bb-field-wrapper">
		...
		<div class="bb-field-wrapper-inner">
			...
			<div class="bb-field-wrap">
				...
				} else if ( 'meeting_occurrence' === bp_get_zoom_meeting_zoom_type() ) {
            ...
            <div class="bp-zoom-meeting-recurring-options <?php echo ! bp_get_zoom_meeting_recurring() ? 'bp-hide': ''; ?>">
                <div class="bb-field-wrap">
                    ...
                    <select name="bp-zoom-meeting-recurrence" id="bp-zoom-meeting-recurrence">
                        <option value="1" <?php echo 1 === $recurrence_type ? 'selected="selected"' : ''; ?>><?php _e( 'Daily', 'buddyboss-pro' ) ?></option>
                        <option value="2" <?php echo 2 === $recurrence_type ? 'selected="selected"' : ''; ?>><?php _e( 'Weekly', 'buddyboss-pro' ) ?></option>
                        <option value="3" <?php echo 3 === $recurrence_type ? 'selected="selected"' : ''; ?>><?php _e( 'Monthly', 'buddyboss-pro' ) ?></option>
                    </select>
                </div>
                        ...
                        <div class="bb-meeting-input-wrap">
                            <select name="bp-zoom-meeting-repeat-interval" id="bp-zoom-meeting-repeat-interval">
                                ...
                                <?php for ( $i = 1; $i <= $repeat_interval_counter; $i++ ) : ?>
                                    <option value="<?php echo $i; ?>" <?php echo $i === $recurrence_repeat_interval ? 'selected="selected"' : ''; ?>><?php echo $i; ?></option>
								<?php endfor; ?>
                            </select>
                            <span id="bp-zoom-meeting-repeat-interval-type"><?php echo $recurrence_label; ?></span>
                        </div>
                    </div>
                    <div class="bb-field-wrap bp-zoom-meeting-occurs-on <?php echo 1 === $recurrence_type || empty( $recurrence_type ) ? 'bp-hide' : ''; ?>">
                        ...
                        <div class="bb-meeting-input-wrap">
                            <div id="bp-zoom-meeting-occurs-on-week" class="<?php echo 2 === $recurrence_type ? '' : 'bp-hide'; ?>">
                                <input type="checkbox" name="bp-zoom-meeting-weekly-days[]" id="bp-zoom-meeting-weekly-days-sun" value="1" class="bs-styled-checkbox" <?php echo in_array( '1', $recurrence_weekly_days ) ? 'checked' : ''; ?>/>
                                <label for="bp-zoom-meeting-weekly-days-sun"><span><?php _e( 'Sun', 'buddyboss-pro' ); ?></span></label>
                                <input type="checkbox" name="bp-zoom-meeting-weekly-days[]" id="bp-zoom-meeting-weekly-days-mon" value="2" class="bs-styled-checkbox" <?php echo in_array( '2', $recurrence_weekly_days ) ? 'checked' : ''; ?>/>
                                <label for="bp-zoom-meeting-weekly-days-mon"><span><?php _e( 'Mon', 'buddyboss-pro' ); ?></span></label>
                                <input type="checkbox" name="bp-zoom-meeting-weekly-days[]" id="bp-zoom-meeting-weekly-days-tue" value="3" class="bs-styled-checkbox" <?php echo in_array( '3', $recurrence_weekly_days ) ? 'checked' : ''; ?>/>
                                <label for="bp-zoom-meeting-weekly-days-tue"><span><?php _e( 'Tue', 'buddyboss-pro' ); ?></span></label>
                                <input type="checkbox" name="bp-zoom-meeting-weekly-days[]" id="bp-zoom-meeting-weekly-days-wed" value="4" class="bs-styled-checkbox" <?php echo in_array( '4', $recurrence_weekly_days ) ? 'checked' : ''; ?>/>
                                <label for="bp-zoom-meeting-weekly-days-wed"><span><?php _e( 'Wed', 'buddyboss-pro' ); ?></span></label>
                                <input type="checkbox" name="bp-zoom-meeting-weekly-days[]" id="bp-zoom-meeting-weekly-days-thu" value="5" class="bs-styled-checkbox" <?php echo in_array( '5', $recurrence_weekly_days ) ? 'checked' : ''; ?>/>
                                <label for="bp-zoom-meeting-weekly-days-thu"><span><?php _e( 'Thu', 'buddyboss-pro' ); ?></span></label>
                                <input type="checkbox" name="bp-zoom-meeting-weekly-days[]" id="bp-zoom-meeting-weekly-days-fri" value="6" class="bs-styled-checkbox" <?php echo in_array( '6', $recurrence_weekly_days ) ? 'checked' : ''; ?>/>
                                <label for="bp-zoom-meeting-weekly-days-fri"><span><?php _e( 'Fri', 'buddyboss-pro' ); ?></span></label>
                                <input type="checkbox" name="bp-zoom-meeting-weekly-days[]" id="bp-zoom-meeting-weekly-days-sat" value="7" class="bs-styled-checkbox" <?php echo in_array( '7', $recurrence_weekly_days ) ? 'checked' : ''; ?>/>
                            ...
                            <div id="bp-zoom-meeting-occurs-on-month" class="<?php echo 3 === $recurrence_type ? '' : 'bp-hide'; ?>">
                                ...
                                <label for="bp-zoom-meeting-occurs-month-day-select">
                                    ...
                                    <select id="bp-zoom-meeting-monthly-day" name="bp-zoom-meeting-monthly-day">
										<?php for ( $i = 1; $i <= 31; $i++ ) : ?>
                                            <option value="<?php echo $i; ?>" <?php selected( $i, $recurrence_monthly_day, true );?>><?php echo $i; ?></option>
										<?php endfor; ?>
                                    </select>
                                ...
                                <label for="bp-zoom-meeting-occurs-month-week-select">
                                    <select id="bp-zoom-meeting-monthly-week" name="bp-zoom-meeting-monthly-week">
                                        <option value="1" <?php echo 1 === $recurrence_monthly_week ? 'selected="selected"' : ''; ?>><?php _e( 'First', 'buddyboss-pro' ); ?></option>
                                        <option value="2" <?php echo 2 === $recurrence_monthly_week ? 'selected="selected"' : ''; ?>><?php _e( 'Second', 'buddyboss-pro' ); ?></option>
                                        <option value="3" <?php echo 3 === $recurrence_monthly_week ? 'selected="selected"' : ''; ?>><?php _e( 'Third', 'buddyboss-pro' ); ?></option>
                                        <option value="4" <?php echo 4 === $recurrence_monthly_week ? 'selected="selected"' : ''; ?>><?php _e( 'Fourth', 'buddyboss-pro' ); ?></option>
                                        <option value="-1" <?php echo -1 === $recurrence_monthly_week ? 'selected="selected"' : ''; ?>><?php _e( 'Last', 'buddyboss-pro' ); ?></option>
                                    </select>
                                    <select id="bp-zoom-meeting-monthly-week-day" name="bp-zoom-meeting-monthly-week-day">
                                        <option value="1" <?php echo 1 === $recurrence_monthly_week_day ? 'selected="selected"' : ''; ?>><?php _e( 'Sun', 'buddyboss-pro' ); ?></option>
                                        <option value="2" <?php echo 2 === $recurrence_monthly_week_day ? 'selected="selected"' : ''; ?>><?php _e( 'Mon', 'buddyboss-pro' ); ?></option>
                                        <option value="3" <?php echo 3 === $recurrence_monthly_week_day ? 'selected="selected"' : ''; ?>><?php _e( 'Tue', 'buddyboss-pro' ); ?></option>
                                        <option value="4" <?php echo 4 === $recurrence_monthly_week_day ? 'selected="selected"' : ''; ?>><?php _e( 'Wed', 'buddyboss-pro' ); ?></option>
                                        <option value="5" <?php echo 5 === $recurrence_monthly_week_day ? 'selected="selected"' : ''; ?>><?php _e( 'Thu', 'buddyboss-pro' ); ?></option>
                                        <option value="6" <?php echo 6 === $recurrence_monthly_week_day ? 'selected="selected"' : ''; ?>><?php _e( 'Fri', 'buddyboss-pro' ); ?></option>
                                        <option value="7" <?php echo 7 === $recurrence_monthly_week_day ? 'selected="selected"' : ''; ?>><?php _e( 'Sat', 'buddyboss-pro' ); ?></option>
                                    </select>
									<?php _e( 'of the month', 'buddyboss-pro' ) ?>
                                </label>
                            </div>
                            ...
                            <div>
                                ...
                                <label for="bp-zoom-meeting-end-date-select">
                                    ...
                                    <div class="bb-field-wrap end-date-picker">
                                        <input type="text" id="bp-zoom-meeting-end-date-time" value="<?php echo false !== $recurrence_end_date_time ? gmdate( 'Y-m-d', strtotime( $recurrence_end_date_time ) ) : gmdate( 'Y-m-d', strtotime( '+7 days' ) ); ?>" name="bp-zoom-meeting-end-date-time" placeholder="yyyy-mm-dd" />
                                    </div>
                                </label>
                            </div>
                            <div>
                                ...
                                <label for="bp-zoom-meeting-end-times-select">
                                    ...
                                    <select id="bp-zoom-meeting-end-times" name="bp-zoom-meeting-end-times">
										<?php for ( $i = 1; $i <= 20; $i++ ) : ?>
                                            <option value="<?php echo $i; ?>" <?php echo $i === $recurrence_end_times || ( empty( $recurrence_end_times ) && 7 === $i ) ? 'selected="selected"' : ''; ?>><?php echo $i; ?></option>
										<?php endfor; ?>
                                    </select>
					...
					<?php if ( ! $disable_registration ) : ?>
                        <div class="bb-field-wrap checkbox-row">
                            ...
                            <div class="bp-zoom-meeting-registration-options <?php echo ! empty( bp_get_zoom_meeting_registration_url() ) && bp_get_zoom_meeting_recurring() ? '' : 'bp-hide'; ?>">
                                <input type="radio" value="1" id="bp-zoom-meeting-registration-type-1" name="bp-zoom-meeting-registration-type" class="bs-styled-radio" <?php checked( 1, bp_get_zoom_meeting_registration_type() ); ?> />
	...
	<footer class="bb-model-footer meeting-item text-right" data-id="<?php bp_zoom_meeting_id(); ?>" data-zoom-type="<?php echo bp_get_zoom_meeting_zoom_type(); ?>" data-action="edit-cancel">
